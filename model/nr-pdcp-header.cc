...
NrPdcpHeader::Deserialize(Buffer::Iterator start)
{
    ...
    // Control/non-data PDUs can appear due to reassembly corner cases.
    // Do not abort simulation here; caller can drop unsupported PDUs.
    if (m_dcBit != DATA_PDU)
    {
        NS_LOG_WARN("Unsupported PDCP PDU type D/C=" << +m_dcBit << " (expected DATA_PDU)");
    }
